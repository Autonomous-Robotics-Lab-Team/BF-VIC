...
namespace franka_example_controllers
{
  ...
  private:
    ...
    double depthDistance = 0.2;
    double depthDistance_filter = 0.2;
    double VirtualRange = 0.2;
    ...
    //打印error[2]
    std::vector<double> error_z_buffer;  // 存储Error[2]数据的缓冲区
    int buffer_size = 50;               // 缓冲区大小，存储50个数据点
    double last_sample_time = 0.0;       // 上次采样的时间
    double sample_interval = 0.05;        // 采样间隔50毫秒
    double error_avg = 0.0;              // 最新100次数据的平均值
  };
